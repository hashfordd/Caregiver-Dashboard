...
const { data: beacons } = await sb
  .from('beacons').select('mac_address,x_canvas,y_canvas,rssi_at_1m').eq('patient_id', PID);
const placed = (beacons || [])
  .filter((b) => b.x_canvas != null && b.y_canvas != null)
  .map((b) => ({ mac: String(b.mac_address), x: +b.x_canvas, y: +b.y_canvas, rssi1m: b.rssi_at_1m == null ? DEFAULT_RSSI1M : +b.rssi_at_1m }));
if (placed.length < 3) {
  console.error(`Only ${placed.length} placed beacons for ${PID} — need 3+ to trilaterate.`);
  process.exit(1);
}
const { data: plan } = await sb
  .from('floor_plans').select('scale_meters_per_pixel').eq('patient_id', PID).eq('is_active', true).maybeSingle();
const SCALE = +plan?.scale_meters_per_pixel;
if (!(SCALE > 0)) {
  console.error('Active floor plan has no usable scale_meters_per_pixel — set the scale on the plan first.');
  process.exit(1);
}

const xs = placed.map((b) => b.x), ys = placed.map((b) => b.y);
const minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys);
const cx = (minX + maxX) / 2, cy = (minY + maxY) / 2, rx = (maxX - minX) * 0.38, ry = (maxY - minY) * 0.38;
...
console.log(`${placed.length} beacons · scale ${SCALE} m/px · walk area ≈ ${((maxX - minX) * SCALE).toFixed(1)} × ${((maxY - minY) * SCALE).toFixed(1)} m`);
...
const c = mqtt.connect(env.MQTT_BROKER_URL, { username: env.MQTT_USERNAME, password: env.MQTT_PASSWORD, reconnectPeriod: 2000 });
let theta = 0, n = 0, paused = false, pauseUntil = 0;
...
function tick() {
  ...
  const px = cx + rx * Math.cos(theta), py = cy + ry * Math.sin(theta);
  ...
  c.publish(TOPIC, JSON.stringify({ v: 1, patient_id: PID, device_id: DID, recorded_at: new Date().toISOString(), ble, wifi: [] }), { qos: 0 });
  if (++n % 5 === 1) {
    console.log(`tick ${n} ${paused ? '[STILL]' : '[walk]'} pos=(${px.toFixed(0)},${py.toFixed(0)}) rssi=[${ble.map((b) => b.rssi).join(', ')}] bat=[${ble.map((b) => b.battery_pct).join(', ')}]`);
  }
}
c.on('connect', () => { console.log('connected; publishing 1 Hz (Ctrl-C to stop)'); tick(); setInterval(tick, 1000); });
c.on('error', (e) => console.error('mqtt error', e.message));
process.on('SIGINT', () => { c.end(true); process.exit(0); });
